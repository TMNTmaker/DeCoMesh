# DeCoMesh — 疎な3Dベクトル場による単一画像からの高速メッシュ再構築

**単一のRGB画像から3Dメッシュ（頂点＋辺＋面）を直接復元する研究プロジェクト。**
ボクセル占有表現に頼らず、3D格子上に疎に配置した2種類のベクトル場（**Edge–Offset Vector Field**）としてメッシュ構造を符号化し、幾何的な後処理のみでメッシュへ復元します。復元処理は 1シーンあたり約30msで動作します。
